...
def ExtractFilefromDir(fpath,mtype='dict',search=[]):
    #global datasets
        ...
        for f in files:
            ...
            if os.path.splitext(name)[1].lower() not in delefile:
                ...
                if os.path.basename(name).lower().endswith('.iso'):
                    get_fileisoinfo(name,mtype=mtype,search=search)
                elif os.path.basename(name).lower().endswith('.zip'):
                    get_filezipinfo(name,mtype=mtype,search=search)
                elif os.path.basename(name).lower().endswith('.rar'):
                    get_filerarinfo(name,mtype=mtype,search=search)
                elif os.path.basename(name).lower().endswith('.tar'):
                    get_filetarinfo(name,mtype=mtype,search=search)
                else:
                    sn=os.path.splitext(os.path.basename(f))[0]
                    for i in search:
                        if sn in i:
                            shutil.copy(os.path.join(root,f),'extract/%s'%f)
    return

def Extractrar(rf):
    rfs=rarfile.RarFile(rf)
    nl=rfs.namelist()
    namedir=os.path.splitext(os.path.basename(rf))[0]
    if len(nl)>0:
        rfs.extractall(namedir+'/')
    return

def Extractzip(rf):
    rfs=zipfile.ZipFile(rf)
    nl=rfs.namelist()
    namedir=os.path.splitext(os.path.basename(rf))[0]
    if len(nl)>0:
        rfs.extractall(namedir+'/')
